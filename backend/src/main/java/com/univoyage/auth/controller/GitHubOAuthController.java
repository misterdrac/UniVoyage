...
public class GitHubOAuthController {
  ...
  public ResponseEntity<ApiResponse<AuthPayload>> githubCallback(HttpServletRequest httpRequest,
      @RequestBody OAuthCallbackRequestDto request, HttpServletResponse response) {
    return handleCallback(httpRequest, request, response, "github");
  }
  ...
  private ResponseEntity<ApiResponse<AuthPayload>> handleCallback(HttpServletRequest httpRequest,
    ...
    AuthPayload payload = githubOAuthService.handleCallback(request.getCode(), stateParam);
    ...
    var user = userRepository.findById(payload.getUser().getId())
        .orElseThrow(() -> new IllegalStateException("User not found after GitHub auth"));
