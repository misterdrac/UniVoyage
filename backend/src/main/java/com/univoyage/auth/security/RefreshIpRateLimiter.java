...
/**
 * Per-IP fixed-window limiter for {@code POST /api/auth/refresh} (separate
 * budget from login).
 */
@Component
public class RefreshIpRateLimiter {
  ...
  public RefreshIpRateLimiter(LoginSecurityProperties properties, Clock clock) {
    this.delegate = new FixedWindowIpRateLimiter(clock, properties.getRefreshIpMaxAttempts(),
        properties.getRefreshIpWindow());
  }
