...
/**
 * Service for handling JWT (JSON Web Tokens) and CSRF (Cross-Site Request
 * Forgery) tokens. This service generates, validates, and extracts claims from
 * JWTs.
 */
@Service
public class JwtService {
  ...
  /**
   * Generating CSRF(Cross-Site Request Forgery) token we call it Token A
   */
  ...
  /**
   * Generating JWT(JSON Web Token) with CSRF claim we call it Token B, JWT is
   * issued by user ID
   */
  public String generateJwtToken(String subject, Map<String, Object> claims, String csrfSecret) {
    ...
    return Jwts.builder().setId(UUID.randomUUID().toString()).setSubject(subject).setIssuer(issuer)
        .setAudience(audience).addClaims(claims).setIssuedAt(Date.from(now))
   ...
   * claims are put into HashMap -> used for testing Only importaint information
   * which token holds is: 1) subject - user ID 2) issuer - univoyage 3) audiance
   * - web 4) CSRF Token - for user 5) issued at - time of issue 6) issued not
   * before - issue is not set before this time 7) experies at - time of issue +
   * TTL of token
   */
  public TokenPair generateForUser(UserEntity user) {
    ...
    // Here we add additional claims if needed, but they are more for testing
    // purposes
    // claims.put("uid", user.getId());
    // claims.put("username", user.getUsername()); // will be commented out if
    // necessary
    // claims.put("email", user.getEmail()); // will be commented out if necessary
    // claims.put("role", user.getRole().name()); // will be commented out if
    // necessary
  ...
  /**
   * Method for parsing token - checks if token is really valid, if mismatched
   * returns error - importaint is that we issue token with field issuer and check
   * that field when validating the token - if not, attackers might use some other
   * token not issued by us
   */
  private Claims parse(String token) {
    try {
      return Jwts.parser().verifyWith((SecretKey) key).requireIssuer(issuer)
  ...
  /**
   * Extracting claim from token using resolver function - this is used for
   * extracting subject, CSRF secret and other claims
   */
  ...
  /**
   * Extracting subject from token - subject is user ID, so we can use it to load
   * user details
   */
  ...
  /**
   * Extracting CSRF secret from token - this is used for checking if CSRF token
   * matches the one in JWT
   */
  ...
  /**
   * Validating token - this method checks if token is valid, not expired and has
   * correct issuer - it does not check if CSRF secret matches, that is done
   * separately
   */
