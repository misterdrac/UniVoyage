...
/**
 * Payload returned after authentication attempts. Contains success status, user
 * details, tokens, and error messages.
 */
@Getter
@Setter
@NoArgsConstructor
...
public class AuthPayload {
  ...
  public static AuthPayload ok(UserDto user, String token, String csrfToken) {
    return AuthPayload.builder().success(true).user(user).token(token).csrfToken(csrfToken).build();
  }
