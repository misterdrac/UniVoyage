...
/**
 * Service for handling authentication-related operations such as registration,
 * login, and profile updates. Interacts with UserRepository for user data
 * access and JwtService for token generation. Handles business logic for user
 * management including password encoding and validation. Uses DTOs for request
 * and response payloads. All methods that modify data are transactional.
 */
@Service
...
public class AuthService {
   ...
   * @return An AuthPayload containing the registered user info and JWT tokens, or
   *         an error message.
   */
  @Transactional
  public AuthPayload register(RegisterRequestDto request) {
    ...
    Country homeCountry = countryRepository.findByIsoCode(request.getCountryCode()).orElseThrow(
        () -> new IllegalArgumentException("Invalid country code: " + request.getCountryCode()));

    Instant now = Instant.now();

    UserEntity newUser = UserEntity.builder().name(request.getName()).surname(request.getSurname())
        .email(request.getEmail()).passwordHash(passwordEncoder.encode(request.getPassword()))
        .country(homeCountry).dateOfRegister(now).dateOfLastSignin(now).role(Role.USER).build();
    ...
    newUser.setUserHobbies(hobbyEntities.stream().map(h -> {
    ...
    newUser.setUserLanguages(languageEntities.stream().map(lang -> {
    ...
    Set<UserVisitedCountry> visitedCountries = Optional.ofNullable(request.getVisitedCountryCodes())
        .orElse(Collections.emptySet()).stream().map(code -> {
          Country visited = countryRepository.findByIsoCode(code).orElseThrow(
              () -> new IllegalArgumentException("Invalid visited country code: " + code));
   ...
   * @return An AuthPayload containing the authenticated user info and JWT tokens,
   *         or an error message.
   */
  @Transactional
   ...
   * @param request
   *          The profile update request containing new user details.
  ...
  public UserDto updateProfile(Long userId, UpdateProfileRequestDto request) {
    ...
    // COUNTRY
    if (request.getCountryCode() != null && !request.getCountryCode().isBlank()) {
      Country country = countryRepository.findByIsoCode(request.getCountryCode()).orElseThrow(
          () -> new IllegalArgumentException("Invalid country code: " + request.getCountryCode()));
    ...
    if (request.getHobbyIds() != null) {
      ...
      Set<Long> existingHobbyIds = user.getUserHobbies().stream().map(uh -> uh.getHobby().getId())
          .collect(Collectors.toSet());

      Set<Hobby> newHobbies = newHobbyIds.stream().filter(id -> !existingHobbyIds.contains(id))
          .map(id -> hobbyRepository.findById(id)
              .orElseThrow(() -> new IllegalArgumentException("Invalid hobby id: " + id)))
          .collect(Collectors.toSet());
    ...
    if (request.getLanguageCodes() != null) {
      ...
      user.getUserLanguages()
          .removeIf(ul -> !newLanguageCodes.contains(ul.getLanguage().getLangCode()));

      Set<String> existingLanguageCodes = user.getUserLanguages().stream()
          .map(ul -> ul.getLanguage().getLangCode()).collect(Collectors.toSet());
    ...
    if (request.getVisitedCountryCodes() != null) {
      ...
      user.getVisitedCountries()
          .removeIf(uvc -> !newCountryCodes.contains(uvc.getCountry().getIsoCode()));

      Set<String> existingCountryCodes = user.getVisitedCountries().stream()
          .map(uvc -> uvc.getCountry().getIsoCode()).collect(Collectors.toSet());

      Set<UserVisitedCountry> newVisitedCountries = newCountryCodes.stream()
          .filter(code -> !existingCountryCodes.contains(code)).map(code -> {
            Country visited = countryRepository.findByIsoCode(code).orElseThrow(
                () -> new IllegalArgumentException("Invalid visited country code: " + code));
      ...
      if (avatarPath.isBlank()) {
        ...
        if (!isValidAvatarUrl(avatarPath)) {
          throw new IllegalArgumentException(
              "Invalid avatar URL. Only avatar-1.png to avatar-20.png from cdn.shadcnstudio.com are allowed.");
        }
  ...
  /**
   * Validates that the avatar URL is from the allowed CDN and matches the
   * expected pattern. Only allows avatar-1.png through avatar-20.png from
   * cdn.shadcnstudio.com
   *
   * @param url
   *          The avatar URL to validate
