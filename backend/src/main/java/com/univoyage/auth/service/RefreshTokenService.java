...
public class RefreshTokenService {
  ...
  /**
   * Persists a new refresh token for the user and returns the raw secret to store
  ...
  public String issueRefreshToken(UserEntity user) {
    ...
    RefreshTokenEntity entity = RefreshTokenEntity.builder().user(user).tokenHash(sha256Hex(raw))
        .expiresAt(now.plusSeconds(refreshTtlSeconds)).createdAt(now).build();
    refreshTokenRepository.save(entity);
    return raw;
  }

  /**
   * Validates the presented refresh token, revokes it (rotation), issues new
