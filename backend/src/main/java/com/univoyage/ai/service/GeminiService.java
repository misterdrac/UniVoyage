...
/**
 * Service for interacting with the Gemini AI API to generate travel itineraries
 * and packing suggestions. Handles prompt construction, API calls, and response
 * parsing. Manages error handling and configuration checks. Uses RestTemplate
 * for HTTP requests and Jackson for JSON processing. Provides methods to
 * generate itineraries and packing lists based on user input. Validates
 * configuration before making API calls. Logs errors for troubleshooting and
 * monitoring.
 */
@Service
@Log4j2
public class GeminiService {
  ...
  public boolean isConfigured() {
    return geminiApiKey != null && !geminiApiKey.isBlank() && geminiModel != null
        && !geminiModel.isBlank();
  }
   ...
   *
   * @param request
   *          The itinerary request containing location, dates, and user
   *          preferences.
   * @return A GeminiResponse containing the generated itinerary or an error
   *         message.
   */
  public GeminiResponse generateItinerary(ItineraryRequest request) {
    ...
    try {
      ...
      return GeminiResponse.error(
          "Oops! Something went wrong while generating your itinerary. Please try again in a moment.");
    }
  }
   ...
   *
   * @param request
   *          The packing request containing destination, dates, and weather
   *          forecast.
   * @return A GeminiResponse containing the packing suggestions or an error
   *         message.
   */
  ...
  /**
   * Generates a budget estimate based on destination, trip duration, and time of
   * year.
   *
   * @param request
   *          The budget estimate request containing destination and dates.
   ...
   * @return A GeminiResponse containing the AI-generated content or an error
   *         message.
   */
  private GeminiResponse callGeminiApi(String prompt) {
    try {
      ...
      log.debug("Gemini API error body (truncated): {}",
          bodyPreviewForDebug(e.getResponseBodyAsString(), 512));
   ...
   *
   * @param responseBody
   *          The raw JSON response body from the Gemini API.
  ...
  private String buildItineraryPrompt(ItineraryRequest request) {
    String dateRange = formatDateLong(request.getDepartureDate()) + " – "
        + formatDateLong(request.getReturnDate());

    String dayAnchors = "";
    if (request.getItineraryDates() != null && !request.getItineraryDates().isEmpty()) {
      dayAnchors = request.getItineraryDates().stream().map(
          d -> "Day " + (request.getItineraryDates().indexOf(d) + 1) + " (" + d.getLabel() + ")")
    ...
    int totalDays = Math
        .max(request.getItineraryDates() != null ? request.getItineraryDates().size() : 1, 1);
    ...
    return """
        ...
        """
        .formatted(totalDays, request.getLocationLabel(), dateRange, dayAnchors, hobbiesContext,
            request.getLocationLabel(), totalDays, request.getLocationLabel())
        .trim();
  }
  ...
  private String buildPackingPrompt(PackingRequest request) {
    return """
        ...
        """
        .formatted(request.getDestinationName(), request.getDepartureDate(),
            request.getReturnDate(), request.getForecastSummary())
        .trim();
  }
  ...
  private String buildBudgetEstimatePrompt(BudgetEstimateRequest request) {
    ...
    return """
        ...
        """
        .formatted(request.getDestinationName(), request.getDestinationLocation(), totalDays, month,
            totalDays, month, request.getDestinationName())
        .trim();
  }
