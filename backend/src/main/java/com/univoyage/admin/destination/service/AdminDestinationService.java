...
/**
 * Service for managing destinations in the admin panel. Provides methods for
 * CRUD operations on destinations. Interacts with DestinationRepository for
 * data access. Handles business logic and data transformation. Throws
 * ResourceNotFoundException for non-existent resources. Uses DTOs for request
 * and response payloads. Supports pagination and searching for listing
 * destinations. All methods that modify data are transactional. Maps
 * DestinationEntity to AdminDestinationResponse DTO. Validates input data
 * before processing.
 */
@Service
...
public class AdminDestinationService {
  ...
  public AdminDestinationPageResponse list(String search, Pageable pageable) {
    ...
    return new AdminDestinationPageResponse(page.getContent().stream().map(this::toDto).toList(),
  ...
  public AdminDestinationResponse patchUpdate(long id, AdminPatchDestinationRequest req) {
    ...
    if (req.location() != null)
      e.setLocation(req.location());
    if (req.continent() != null)
      e.setContinent(req.continent());
    if (req.countryCode() != null)
      e.setCountry(resolveCountry(req.countryCode()));
    if (req.imageUrl() != null)
      e.setImageUrl(req.imageUrl());
    if (req.imageAlt() != null)
      e.setImageAlt(req.imageAlt());
    if (req.overview() != null)
      e.setOverview(req.overview());
    if (req.budgetPerDay() != null)
      e.setBudgetPerDay(req.budgetPerDay());
    if (req.whyVisit() != null)
      e.setWhyVisit(req.whyVisit());
    if (req.studentPerks() != null)
      e.setStudentPerks(req.studentPerks());
    if (req.averageRating() != null)
      e.setAverageRating(toBigDecimal(req.averageRating()));
  ...
  private AdminDestinationResponse toDto(DestinationEntity e) {
    ...
    return new AdminDestinationResponse(e.getId(), e.getName(), e.getLocation(), e.getContinent(),
        iso, e.getImageUrl(), e.getImageAlt(), e.getOverview(), e.getBudgetPerDay(),
        e.getWhyVisit(), e.getStudentPerks(),
        e.getAverageRating() == null ? null : e.getAverageRating().doubleValue(), e.getCreatedAt(),
        e.getUpdatedAt());
  }
