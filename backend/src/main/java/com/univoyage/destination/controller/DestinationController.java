...
/**
 * Controller for managing travel destinations. Provides endpoints to retrieve
 * all destinations, search destinations, and create new destinations. All
 * endpoints are prefixed with /api/destinations. Uses DestinationService for
 * business logic. Returns responses wrapped in ApiResponse for consistent API
 * structure.
 */
@RestController
...
public class DestinationController {
  ...
  /**
   * Published text reviews for a destination (moderation-approved only).
   * Paginated for the public site.
   */
  @GetMapping("/{destinationId}/reviews")
  public ResponseEntity<ApiResponse<Map<String, Object>>> listPublishedReviews(
      @PathVariable Long destinationId, @PageableDefault(size = 10) Pageable pageable) {
    DestinationReviewsPageResponse page = destinationReviewService
        .listPublishedReviews(destinationId, pageable);
    return ResponseEntity.ok(ApiResponse.ok(Map.of("reviews", page)));
  }

  @PostMapping
  public ResponseEntity<ApiResponse<Map<String, Object>>> create(
      @Valid @RequestBody CreateDestinationRequest req) {
