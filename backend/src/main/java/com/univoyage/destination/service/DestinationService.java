...
/**
 * Service for managing travel destinations. Provides methods to retrieve,
 * search, and create destination records.
 */
@Service
...
public class DestinationService {
  ...
  public List<DestinationResponse> search(String query) {
    ...
    return destinationRepository
        .findTop25ByNameContainingIgnoreCaseOrLocationContainingIgnoreCase(query, query).stream()
   ...
   *
   * @param req
   *          The CreateDestinationRequest DTO containing destination details.
  ...
  public DestinationResponse create(CreateDestinationRequest req) {
    DestinationEntity entity = destinationRepository
        .findByNameAndLocation(req.getName().trim(), req.getLocation().trim()).orElseGet(
            () -> destinationRepository.save(DestinationEntity.builder().name(req.getName().trim())
                .location(req.getLocation().trim()).continent(req.getContinent().trim())
                .country(Country.builder().isoCode(req.getCountryCode().trim()).build())
   ...
   *
   * @param d
   *          The DestinationEntity to convert.
  ...
  private DestinationResponse toDto(DestinationEntity d) {
    ...
    if (d.getCountry() != null) {
      countryResponse = DestinationResponse.CountryResponse.builder()
          .isoCode(d.getCountry().getIsoCode()).countryName(d.getCountry().getCountryName())
          .currencyCode(d.getCountry().getCurrencyCode())
          .currencyName(d.getCountry().getCurrencyName()).build();
    }

    return DestinationResponse.builder().id(d.getId()).title(d.getName()).location(d.getLocation())
        .continent(d.getContinent()).country(countryResponse).imageUrl(d.getImageUrl())
        .imageAlt(d.getImageAlt()).overview(d.getOverview()).budgetPerDay(d.getBudgetPerDay())
        .whyVisit(d.getWhyVisit()).studentPerks(d.getStudentPerks())
        .averageRating(d.getAverageRating() == null ? null : d.getAverageRating().doubleValue())
        .travellerRatingAverage(d.getTravellerRatingAverage() == null
            ? null
            : d.getTravellerRatingAverage().doubleValue())
