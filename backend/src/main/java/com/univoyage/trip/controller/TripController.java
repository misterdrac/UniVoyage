...
public class TripController {
  ...
  @PostMapping
  public ResponseEntity<ApiResponse<Map<String, Object>>> createTrip(
  ...
  public ResponseEntity<ApiResponse<Object>> saveBudget(@PathVariable Long tripId,
  ...
  public ResponseEntity<ApiResponse<Map<String, Object>>> getItinerary(@PathVariable Long tripId) {
    ...
    return ResponseEntity
        .ok(ApiResponse.ok(Map.of("itinerary", itinerary == null ? Map.of() : itinerary)));
  }

  @PutMapping("/{tripId}/itinerary")
  public ResponseEntity<ApiResponse<Object>> saveItinerary(@PathVariable Long tripId,
      @RequestBody Object payload) {
    Long userId = currentUser.id();
  ...
  public ResponseEntity<ApiResponse<Object>> saveAccommodation(@PathVariable Long tripId,
  ...
  public ResponseEntity<ApiResponse<Map<String, Object>>> getAccommodation(
      @PathVariable Long tripId) {
    Long userId = currentUser.id();
    TripAccommodationResponse acc = tripService.getAccommodation(userId, tripId);
    return ResponseEntity.ok(ApiResponse.ok(Map.of("accommodation", acc == null ? Map.of() : acc)));
  }
  ...
  public ResponseEntity<ApiResponse<Map<String, Object>>> getCurrency(@PathVariable Long tripId) {
    ...
    try {
      ...
      log.debug(
          ...
          tripId, userId, currency.getBaseCurrencyCode(), currency.getDestinationCurrencyCode());
      return ResponseEntity.ok(ApiResponse.ok(Map.of("currency", currency)));
    } catch (RuntimeException ex) {
      // Let GlobalExceptionHandler translate the error, but log it here for
      // observability.
      log.warn("Trip currency tab failed: tripId={} userId={} reason={}", tripId, userId,
          ex.getMessage());
      throw ex;
    }
  }
  ...
  public ResponseEntity<ApiResponse<Map<String, Object>>> submitTripRating(
      @PathVariable Long tripId, @Valid @RequestBody TripTravellerRatingRequest request,
      HttpServletRequest httpRequest) {
    long ipRetry = tripRatingIpRateLimiter
        .tryConsumeOrRetryAfterSeconds(ClientIpResolver.resolve(httpRequest));
    if (ipRetry >= 0) {
      return ResponseEntity.status(HttpStatus.TOO_MANY_REQUESTS)
          .header(HttpHeaders.RETRY_AFTER, String.valueOf(ipRetry)).body(ApiResponse
              .fail("Too many rating submissions from this network. Please try again later."));
    }
    ...
    if (userRetry >= 0) {
      return ResponseEntity.status(HttpStatus.TOO_MANY_REQUESTS)
          .header(HttpHeaders.RETRY_AFTER, String.valueOf(userRetry)).body(ApiResponse
              .fail("Too many rating submissions for this account. Please try again later."));
    }

    TripTravellerRatingResponse rating = tripTravellerRatingService.submitRating(userId, tripId,
        request);
