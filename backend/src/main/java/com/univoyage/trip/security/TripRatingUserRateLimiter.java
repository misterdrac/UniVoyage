...
/**
 * Per-authenticated-user fixed-window limiter for {@code POST
 * /api/trips/{tripId}/rating}.
 */
@Component
public class TripRatingUserRateLimiter {
  ...
  public TripRatingUserRateLimiter(TripRatingLimitProperties properties, Clock clock) {
    this.delegate = new FixedWindowIpRateLimiter(clock, properties.getUserMaxAttempts(),
        properties.getUserWindow());
  }

  /**
   * @return -1 if the attempt is allowed; otherwise seconds until the current
   *         window resets
   */
