...
public class GeoapifyService {
  ...
  private String geocodeCity(String city) {
    String url = String.format("%s?text=%s&type=city&limit=1&apiKey=%s", GEOCODE_URL,
        URLEncoder.encode(city, StandardCharsets.UTF_8), geoapifyApiKey);
  ...
  private List<PointOfInterest> searchPlacesInCity(String placeId, int limit) {
    ...
    String url = String.format("%s?categories=%s&filter=place:%s&limit=%d&apiKey=%s", PLACES_URL,
        categories, placeId, Math.min(limit, 22), geoapifyApiKey);
    ...
    try {
      ...
      return response.features.stream().map(this::mapToPointOfInterest)
  ...
  private PointOfInterest mapToPointOfInterest(PlaceFeature feature) {
    ...
    poi.setCategory(PlaceCategoryMapper.extractCategory(props.categories, props.name, historicType,
        description));
