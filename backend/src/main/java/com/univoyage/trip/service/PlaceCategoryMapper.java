 ...
 * Provides comprehensive categorization to avoid generic "Attraction" or
 * "Historic Site" defaults.
 */
public class PlaceCategoryMapper {
   ...
   *
   * @param categories
   *          List of category strings from Geoapify API
   * @param placeName
   *          Name of the place
   * @param historicType
   *          Historic type from API (e.g., "castle", "monument")
   * @param description
   *          Description text (may contain category keywords)
   * @return User-friendly category name
   */
  public static String extractCategory(List<String> categories, String placeName,
      String historicType, String description) {
  ...
  private static String getCategoryFromName(String nameLower) {
    ...
    // Squares and Plazas
    if (nameLower.contains("square") || nameLower.contains("plaza") || nameLower.contains("platz")
        || nameLower.contains("piazza") || nameLower.contains("trg")
        || nameLower.contains("place")) {
      return "Square";
    }
  ...
  private static boolean isReligiousCategory(String category) {
    return category.contains("church") || category.contains("cathedral")
        || category.contains("religious") || category.contains("shrine")
        || category.contains("basilica") || category.contains("temple")
        || category.contains("mosque") || category.contains("synagogue")
        || category.contains("chapel") || category.contains("abbey")
        || category.contains("monastery") || category.contains("convent")
        || category.contains("sanctuary") || category.contains("place_of_worship");
  }
  ...
  private static boolean isReligiousName(String name) {
    return name.contains("cathedral") || name.contains("shrine") || name.contains("basilica")
        || name.contains("temple") || name.contains("mosque") || name.contains("synagogue")
        || name.contains("chapel") || name.contains("abbey") || name.contains("monastery")
        || name.contains("convent") || name.contains("sanctuary") || name.contains("church")
  ...
  private static boolean hasMoreSpecificCategory(List<String> categories, String currentCategory) {
    ...
    for (String cat : categories) {
      ...
      if (!catLower.equals(current) && (catLower.contains("castle") || catLower.contains("museum")
