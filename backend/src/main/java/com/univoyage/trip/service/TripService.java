...
public class TripService {
   ...
   * @param req
   *          The request object containing trip details.
  ...
  public TripResponse createTrip(Long userId, CreateTripRequest req) {
    ...
    TripEntity trip = TripEntity.builder().userId(userId).destination(dest)
        .departureDate(LocalDate.parse(req.getDepartureDate()))
        .returnDate(LocalDate.parse(req.getReturnDate())).status("planned").createdAt(Instant.now())
        .build();
   ...
   *
   * @param userId
   *          The ID of the user whose trips are to be retrieved.
   * @return A list of TripResponse objects.
   */
  @Transactional(readOnly = true)
  public List<TripResponse> getTrips(Long userId) {
    return tripRepository.findAllByUserIdOrderByCreatedAtDesc(userId).stream().map(this::toResponse)
        .toList();
  }
   ...
   *
   * @param userId
   *          The ID of the user deleting the trip.
   * @param tripId
   ...
   *
   * @param userId
   *          The ID of the user requesting the budget.
   * @param tripId
   *          The ID of the trip whose budget is to be retrieved.
   * @return The budget object, or null if not found.
   */
  @Transactional(readOnly = true)
  public Object getBudget(Long userId, Long tripId) {
    ensureOwner(userId, tripId);
    return tripBudgetRepository.findById(tripId).map(e -> readJson(e.getPayload())).orElse(null);
  }
   ...
   *
   * @param userId
   *          The ID of the user saving the budget.
   * @param tripId
   *          The ID of the trip whose budget is to be saved.
   * @param payload
   *          The budget data to be saved.
   */
  @Transactional
  public void saveBudget(Long userId, Long tripId, Object payload) {
    ensureOwner(userId, tripId);
    tripBudgetRepository.save(TripBudgetEntity.builder().tripId(tripId).payload(writeJson(payload))
   ...
   *
   * @param userId
   *          The ID of the user requesting the itinerary.
   * @param tripId
   *          The ID of the trip whose itinerary is to be retrieved.
   * @return The itinerary object, or null if not found.
   */
  @Transactional(readOnly = true)
  public Object getItinerary(Long userId, Long tripId) {
    ensureOwner(userId, tripId);
    return tripItineraryRepository.findById(tripId).map(e -> readJson(e.getPayload())).orElse(null);
  }
   ...
   *
   * @param userId
   *          The ID of the user saving the itinerary.
   * @param tripId
   *          The ID of the trip whose itinerary is to be saved.
   * @param payload
   *          The itinerary data to be saved.
   */
  @Transactional
  public void saveItinerary(Long userId, Long tripId, Object payload) {
    ensureOwner(userId, tripId);
    tripItineraryRepository.save(TripItineraryEntity.builder().tripId(tripId)
        .payload(writeJson(payload)).updatedAt(Instant.now()).build());
  }
   ...
   *
   * @param userId
   *          The ID of the user requesting the accommodation details.
   * @param tripId
   *          The ID of the trip whose accommodation details are to be retrieved.
  ...
  public TripAccommodationResponse getAccommodation(Long userId, Long tripId) {
    ...
    return tripAccommodationRepository.findById(tripId)
        .map(e -> TripAccommodationResponse.builder().tripId(tripId)
            .accommodationName(e.getAccommodationName())
            .accommodationAddress(e.getAccommodationAddress())
            .accommodationPhone(e.getAccommodationPhone()).updatedAt(e.getUpdatedAt()).build())
        .orElse(null);
  }
   ...
   *
   * @param userId
   *          The ID of the user saving the accommodation details.
   * @param tripId
   *          The ID of the trip whose accommodation details are to be saved.
   * @param req
   *          The TripAccommodationRequest object containing accommodation
   *          details.
   */
  @Transactional
  public void saveAccommodation(Long userId, Long tripId, TripAccommodationRequest req) {
    ...
    TripAccommodationEntity entity = TripAccommodationEntity.builder().tripId(tripId)
        .accommodationName(req.getAccommodationName())
        .accommodationAddress(req.getAccommodationAddress())
        .accommodationPhone(req.getAccommodationPhone()).updatedAt(Instant.now()).build();
   ...
   *           if the trip does not belong to the user.
   */
   ...
   *
   * @param t
   *          The TripEntity to convert.
   * @return The corresponding TripResponse.
   */
  private TripResponse toResponse(TripEntity t) {
    return TripResponse.builder().id(t.getId()).destinationId(t.getDestination().getId())
        .destinationName(t.getDestination().getName())
        .destinationLocation(t.getDestination().getLocation())
        .departureDate(t.getDepartureDate().toString()).returnDate(t.getReturnDate().toString())
        .createdAt(t.getCreatedAt().toString()).status(t.getStatus()).build();
  }
  ...
  private Object readJson(String raw) {
    try {
      ...
      throw new IllegalStateException("Invalid stored JSON");
    }
  }
