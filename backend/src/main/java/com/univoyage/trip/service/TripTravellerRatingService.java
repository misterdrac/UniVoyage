...
public class TripTravellerRatingService {
  ...
  /**
   * Recomputes {@code destinations.traveller_rating_*} from
   * <strong>approved</strong> ratings only (used after moderation actions).
   */
  @Transactional
  ...
  public TripTravellerRatingResponse getRating(Long userId, Long tripId) {
    ...
    return ratingRepository.findByTripIdAndUserId(tripId, userId).map(this::toResponse)
        .orElse(null);
  }

  @Transactional
  public TripTravellerRatingResponse submitRating(Long userId, Long tripId,
      TripTravellerRatingRequest request) {
    ...
    Optional<TripTravellerRatingEntity> existingOpt = ratingRepository.findByTripIdAndUserId(tripId,
        userId);
    String previousComment = existingOpt.map(TripTravellerRatingEntity::getComment)
        .map(TripTravellerRatingService::normalizeComment).orElse(null);
    ReviewModerationStatus previousStatus = existingOpt
        .map(TripTravellerRatingEntity::getModerationStatus).orElse(null);

    TripTravellerRatingEntity entity = existingOpt
        .orElseGet(() -> TripTravellerRatingEntity.builder().trip(trip).user(userRef).build());
    ...
    } else if (previousStatus == ReviewModerationStatus.APPROVED
        && Objects.equals(previousComment, comment)) {
      entity.setModerationStatus(ReviewModerationStatus.APPROVED);
    } else {
      entity.setModerationStatus(ReviewModerationStatus.PENDING);
    }
  ...
  private TripTravellerRatingResponse toResponse(TripTravellerRatingEntity e) {
    ...
    return TripTravellerRatingResponse.builder()
        .stars(e.getStars() == null ? 0 : e.getStars().intValue()).comment(e.getComment())
        .updatedAt(e.getUpdatedAt()).moderationStatus(st.name()).build();
  }
}
