...
public class WeatherService {
  ...
  public void init() {
    if (openWeatherApiKey == null || openWeatherApiKey.isBlank()) {
      ...
      log.info("OpenWeather API key configured successfully (length: {})",
  ...
  /**
   * Get current weather - returns processed data matching frontend WeatherData
   * format
   */
  public WeatherResponse getCurrentWeather(String city, String country) {
    ...
    String url = String.format("%s?q=%s&APPID=%s&units=metric", CURRENT_WEATHER_URL,
        URLEncoder.encode(queryLocation, StandardCharsets.UTF_8), openWeatherApiKey);
  ...
  /**
   * Get current weather by coordinates - returns processed data matching frontend
   * WeatherData format
   */
  public WeatherResponse getCurrentWeatherByCoordinates(double latitude, double longitude) {
    ...
    String url = String.format("%s?lat=%.4f&lon=%.4f&APPID=%s&units=metric", CURRENT_WEATHER_URL,
  ...
  public Map<String, Object> getForecast(String city, String country) {
    ...
    String url = String.format("%s?q=%s&APPID=%s&units=metric", FORECAST_URL,
        URLEncoder.encode(queryLocation, StandardCharsets.UTF_8), openWeatherApiKey);
  ...
  private WeatherResponse mapToWeatherResponse(OpenWeatherResponse response) {
    ...
    weatherResponse
        .setDateTime(formatDateTimeForTimezone(response.timezone != null ? response.timezone : 0));
    weatherResponse.setDay(response.weather != null && !response.weather.isEmpty()
        && response.weather.get(0).icon != null && response.weather.get(0).icon.contains("d"));
  ...
  public static class WeatherResponse {
    ...
    private String weatherType; // 'clear' | 'clouds' | 'rain' | 'snow' | 'thunderstorm' | 'mist' |
                                // 'unknown'
