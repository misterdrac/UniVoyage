...
public class TripCurrencyService {
  ...
  public TripCurrencyResponse getTripCurrency(Long userId, Long tripId) {
    ...
    log.debug(
        ...
        tripId, destinationCountry.getIsoCode(), destinationCurrencyCode, destinationCurrencyName);
    ...
    if (user.getCountry() != null && user.getCountry().getCurrencyCode() != null) {
      ...
      log.debug("Using user's country currency as base: userId={} baseCurrency={}", userId,
          baseCurrencyCode);
    } else {
      log.debug("User country/currency missing; defaulting base currency to EUR: userId={}",
          userId);
    }

    log.debug("Base currency={} Destination currency={}", baseCurrencyCode,
        destinationCurrencyCode);

    double exchangeRate = currencyRateService.getRate(baseCurrencyCode, destinationCurrencyCode);

    log.info("Computed FX rate: {} -> {} = {}", baseCurrencyCode, destinationCurrencyCode,
        exchangeRate);
    log.info("Trip currency fetch completed: tripId={} userId={} durationMs={}", tripId, userId,
        (System.currentTimeMillis() - startedAt));

    return TripCurrencyResponse.builder().destinationCurrencyCode(destinationCurrencyCode)
        .destinationCurrencyName(destinationCurrencyName).baseCurrencyCode(baseCurrencyCode)
