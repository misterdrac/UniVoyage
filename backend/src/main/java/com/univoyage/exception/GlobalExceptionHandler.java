...
/**
 * Global exception handler for the application. Maps domain and framework
 * exceptions to HTTP status codes and {@link ApiResponse} JSON bodies so
 * clients never receive raw stack traces or non-JSON error pages from the API
 * layer.
 */
@ControllerAdvice
@Log4j2
public class GlobalExceptionHandler {
   ...
   *
   * @param ex
   *          The ResourceNotFoundException instance.
   * @param request
   ...
   *
   * @param ex
   *          The MethodArgumentNotValidException instance.
   * @param request
   *          The current web request.
  ...
  public ResponseEntity<ApiResponse<Map<String, String>>> handleValidationExceptions(
    ...
    ApiResponse<Map<String, String>> response = ApiResponse.of(false, errors,
  ...
  /**
   * Handles illegal client input and business rule violations expressed as
  ...
  /**
   * Handles invalid path or query parameter types (e.g. non-numeric path segment
   * where a number is expected).
   */
  @ExceptionHandler(MethodArgumentTypeMismatchException.class)
  public ResponseEntity<ApiResponse<Object>> handleMethodArgumentTypeMismatchException(
      MethodArgumentTypeMismatchException ex, WebRequest request) {
    String msg = String.format("Invalid value for parameter '%s'.",
  ...
  public ResponseEntity<ApiResponse<Object>> handleMissingServletRequestParameterException(
    ...
    String msg = String.format("Required request parameter '%s' is missing.",
        ex.getParameterName());
    return new ResponseEntity<>(ApiResponse.fail(msg), HttpStatus.BAD_REQUEST);
  }
   ...
   *
   * @param ex
   *          The IllegalStateException instance.
   * @param request
   *          The current web request.
   * @return A ResponseEntity containing the ApiResponse with error message.
   */
  ...
  /**
   * Fallback for uncaught runtime failures (e.g. external API errors). Response
   * body stays generic; details are logged server-side only.
   */
  ...
  /**
   * Last-resort handler so no exception escapes as a non-JSON servlet error for
   * API requests.
   */
