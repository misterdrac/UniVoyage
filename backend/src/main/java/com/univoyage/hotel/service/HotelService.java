...
/**
 * Service for interacting with the Amadeus Hotel API. Handles authentication,
 * token management, and hotel searches by city.
 */
@Service
@Log4j2
public class HotelService {
  ...
  public List<HotelResponse> searchHotels(String city) {
    if (!isConfigured()) {
      throw new IllegalStateException(
          "Hotel API is not configured. Please set AMADEUS_API_KEY and AMADEUS_API_SECRET.");
    }
  ...
  private String refreshToken() {
    try {
      ...
      ResponseEntity<String> response = restTemplate.exchange(AMADEUS_AUTH_URL, HttpMethod.POST,
          entity, String.class);
      ...
      log.debug("Amadeus auth error body (truncated): {}",
  ...
  private List<HotelResponse> fetchHotels(String city, String token) {
    try {
      ...
      String url = AMADEUS_HOTEL_LIST_URL + "?cityCode=" + cityCode
      ...
      log.debug("Amadeus hotel API error body (truncated): {}",
          bodyPreviewForDebug(e.getResponseBodyAsString(), 512));
    ...
    try {
      ...
      if (dataArray.isArray()) {
        for (JsonNode hotelNode : dataArray) {
          ...
          hotels.add(HotelResponse.builder().hotelName(hotelName).hotelId(hotelId).build());
  ...
  private String fetchCityCodeFromAmadeus(String city, String token) {
    try {
      String url = AMADEUS_CITY_SEARCH_URL + "?keyword="
          + URLEncoder.encode(city, StandardCharsets.UTF_8) + "&max=1";
      ...
      log.debug("Amadeus city search error body (truncated): {}",
          bodyPreviewForDebug(e.getResponseBodyAsString(), 512));
      return null;
    } catch (Exception e) {
