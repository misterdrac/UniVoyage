...
public class QuizService {
  ...
  public QuizRecommendationResponse recommend(QuizRequest request) {
    ...
    if (allDestinations.isEmpty()) {
      return QuizRecommendationResponse.builder()
          .intro("We couldn't find any destinations at the moment.").recommendations(List.of())
          .closingNote("Please try again later.").build();
    }
    ...
    if (!aiResponse.isSuccess() || aiResponse.getContent() == null) {
      ...
      return QuizRecommendationResponse.builder()
          .intro("We had trouble generating your recommendations.").recommendations(List.of())
  ...
  private List<DestinationEntity> preFilter(List<DestinationEntity> all, QuizRequest request) {
    List<DestinationEntity> filtered = all.stream()
        ...
        .filter(d -> matchesBudget(d, request.getBudget())).toList();
    ...
    List<DestinationEntity> byContinent = all.stream()
        .filter(d -> matchesContinent(d, request.getContinent())).toList();
  ...
  private String buildPrompt(QuizRequest request, List<DestinationEntity> destinations) {
    ...
    for (DestinationEntity d : destinations) {
      catalog.append(String.format(
          ...
          d.getId(), escape(d.getName()), escape(d.getLocation()), escape(d.getContinent()),
    ...
    return """
        ...
        """
        .formatted(request.getBudget(), request.getClimate(), request.getActivityType(),
            request.getContinent(), request.getTravelStyle(), catalog.toString().trim())
        .trim();
  }

  private QuizRecommendationResponse parseResponse(String content,
    ...
    try {
      ...
      if (recsNode.isArray()) {
        for (JsonNode recNode : recsNode) {
          ...
          recs.add(
              RecommendedDestination.builder().destinationId(entity.getId()).name(entity.getName())
                  .location(entity.getLocation()).continent(entity.getContinent())
                  .imageUrl(entity.getImageUrl()).budgetPerDay(entity.getBudgetPerDay())
                  .matchReason(
                      recNode.path("matchReason").asText("A great match for your preferences!"))
                  .highlights(highlights).build());
        }
      }

      return QuizRecommendationResponse.builder().intro(intro).recommendations(recs)
          .closingNote(closingNote).build();
      ...
      return QuizRecommendationResponse.builder()
          ...
          .recommendations(List.of()).closingNote("Please try the quiz again.").build();
    }
  }
