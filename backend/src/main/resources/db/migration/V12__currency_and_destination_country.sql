-- Currency metadata on countries; link destinations to countries for trip FX.
...
-- HK is seeded in V9; ensure currency row exists after ALTER adds columns.
INSERT INTO countries (iso_code, country_name, currency_code, currency_name)
VALUES ('HK', 'Hong Kong', 'HKD', 'Hong Kong Dollar')
ON CONFLICT (iso_code) DO UPDATE SET
    currency_code = EXCLUDED.currency_code,
    currency_name = EXCLUDED.currency_name;

UPDATE countries AS c
SET currency_code = v.currency_code,
...
FROM (VALUES
    ...
    ('VU', 'VUV', 'Vanuatu vatu'),
    ('VA', 'EUR', 'Euro'),
    ('VE', 'VES', 'Venezuelan bolivar'),
    ('VN', 'VND', 'Vietnamese dong'),
    ('YE', 'YER', 'Yemeni rial'),
    ('ZM', 'ZMW', 'Zambian kwacha'),
    ('ZW', 'ZWL', 'Zimbabwean dollar')
) AS v(iso_code, currency_code, currency_name)
WHERE c.iso_code = v.iso_code;
