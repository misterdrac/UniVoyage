...
CREATE TABLE users (
    ...
    -- Normalized fields: References a separate 'countries' table (recommended)
    country_of_origin_code CHAR(3),

    profile_image_data BYTEA,
    date_of_register   TIMESTAMPTZ        NOT NULL DEFAULT CURRENT_TIMESTAMP, -- Use CURRENT_TIMESTAMP for clarity
...
COMMENT ON COLUMN users.country_of_origin_code IS 'Reference to the user''s country of origin (ISO 3-letter code).';
