...
/**
 * Full-stack integration tests for {@link GeminiController} with
 * {@link GeminiService} mocked.
 */
@SpringBootTest
...
class GeminiControllerIntegrationTest {
  ...
  void getStatusAuthenticated_returnsPayload() throws Exception {
    ...
    mockMvc.perform(get("/api/ai/status")).andExpect(status().isOk())
  ...
  void postItinerary_success() throws Exception {
    ...
    mockMvc
        .perform(post("/api/ai/itinerary").contentType(MediaType.APPLICATION_JSON)
  ...
  void postPacking_success() throws Exception {
    ...
    mockMvc
        .perform(post("/api/ai/packing").contentType(MediaType.APPLICATION_JSON)
            .content(objectMapper.writeValueAsString(req)))
        .andExpect(status().isOk()).andExpect(jsonPath("$.success").value(true))
  ...
  void postBudgetEstimate_success() throws Exception {
    ...
    mockMvc
        .perform(post("/api/ai/budget-estimate").contentType(MediaType.APPLICATION_JSON)
            .content(objectMapper.writeValueAsString(req)))
        .andExpect(status().isOk()).andExpect(jsonPath("$.success").value(true))
  ...
  void postBudgetEstimate_validationFails() throws Exception {
    mockMvc
        .perform(post("/api/ai/budget-estimate").contentType(MediaType.APPLICATION_JSON).content("""
