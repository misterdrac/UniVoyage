...
/**
 * Full-stack integration tests for {@link HotelController} with
 * {@link HotelService} mocked.
 */
@SpringBootTest
...
class HotelControllerIntegrationTest {
  ...
  void hotelSearchOk() throws Exception {
    List<HotelResponse> many = List.of(HotelResponse.builder().hotelName("A").hotelId("1").build(),
    ...
    mockMvc.perform(get("/api/hotels/search").param("city", "Berlin").param("limit", "2"))
  ...
  void hotelSearchMissingCity_returns400() throws Exception {
    mockMvc.perform(get("/api/hotels/search")).andExpect(status().isBadRequest());
  }
  ...
  void hotelSearchBadGateway() throws Exception {
    ...
    mockMvc.perform(get("/api/hotels/search").param("city", "W")).andExpect(status().isBadGateway())
  ...
  void hotelStatus() throws Exception {
    ...
    mockMvc.perform(get("/api/hotels/status")).andExpect(status().isOk())
