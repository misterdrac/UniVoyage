...
/**
 * Full-stack integration tests for {@link AdminDestinationController}. Servlet
 * filters are disabled so role checks are not exercised here; focus is
 * controller, validation, and persistence via
 * {@link com.univoyage.admin.destination.service.AdminDestinationService}.
 */
@SpringBootTest
...
class AdminDestinationControllerIntegrationTest {
  ...
  void getDestination_ok() throws Exception {
    ...
    mockMvc.perform(get("/api/admin/destinations/{id}", id)).andExpect(status().isOk())
        .andExpect(jsonPath("$.success").value(true)).andExpect(jsonPath("$.data.id").value(id));
  }
  ...
  void createDestination_validationFails() throws Exception {
    mockMvc
        .perform(post("/api/admin/destinations").contentType(MediaType.APPLICATION_JSON)
  ...
  void patchDestination_ok() throws Exception {
    DestinationEntity dest = destinationRepository.findAll(PageRequest.of(0, 1)).getContent()
        .getFirst();
    String newName = "AdminPatchName-" + dest.getId();

    AdminPatchDestinationRequest body = new AdminPatchDestinationRequest(newName, null, null, null,
        null, null, null, null, null, null, null);

    mockMvc
        .perform(patch("/api/admin/destinations/{id}", dest.getId())
  ...
  void patchDestination_notFound() throws Exception {
    AdminPatchDestinationRequest body = new AdminPatchDestinationRequest("X", null, null, null,
        null, null, null, null, null, null, null);

    mockMvc
        .perform(patch("/api/admin/destinations/{id}", 9_999_999_999L)
            .contentType(MediaType.APPLICATION_JSON).content(objectMapper.writeValueAsString(body)))
        .andExpect(status().isNotFound()).andExpect(jsonPath("$.success").value(false));
  }
  ...
  void putDestination_notFound() throws Exception {
    AdminUpdateDestinationRequest body = new AdminUpdateDestinationRequest("N", "Loc", "EU", "DE",
        null, null, null, null, null, java.util.List.of(), null);

    mockMvc
        .perform(put("/api/admin/destinations/{id}", 9_999_999_999L)
            .contentType(MediaType.APPLICATION_JSON).content(objectMapper.writeValueAsString(body)))
        .andExpect(status().isNotFound()).andExpect(jsonPath("$.success").value(false));
  }
