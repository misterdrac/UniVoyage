...
/**
 * Full-stack integration tests for {@link UserController}. The real security
 * filter chain runs; authenticated tests install a {@link UserEntity} principal
 * via
...
class UserControllerIntegrationTest {
  ...
  void updateProfileUnauthenticated_returns401() throws Exception {
    mockMvc
        .perform(patch("/api/user/profile").contentType(MediaType.APPLICATION_JSON).content("{}"))
  ...
  void updateProfileSuccess() throws Exception {
    ...
    mockMvc
        .perform(patch("/api/user/profile").with(securityContextFor(user))
    ...
    mockMvc
        .perform(patch("/api/user/profile").with(securityContextFor(user))
            ...
            .content(objectMapper
                .writeValueAsString(Map.of("profileImagePath", "https://evil.example.com/x.png"))))
