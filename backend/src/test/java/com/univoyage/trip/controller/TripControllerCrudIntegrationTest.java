...
/**
 * Integration tests for {@link TripController} trip CRUD, budget, itinerary,
 * and accommodation endpoints.
 * <p>
 * Servlet filters are disabled to focus on controller and service integration;
 * the authenticated user is supplied via a
 * {@link org.springframework.boot.test.mock.mockito.MockBean @MockBean}
 * {@link CurrentUser}. Countries are loaded from Flyway-seeded data
 * ({@link #country(String)}). Each test runs in a transaction that rolls back,
 * keeping the shared test database clean.
 * </p>
 */
@SpringBootTest
...
=======
  ...
  /**
   * {@code POST /api/trips} persists a trip for the mocked current user and
   * echoes destination id in the JSON payload; repository state is asserted after
   * the request.
   */
  @Test
  @DisplayName("POST /api/trips creates trip for current user")
  void createTrip() throws Exception {
    ...
    mockMvc
        .perform(post("/api/trips").contentType(MediaType.APPLICATION_JSON)
            .content(objectMapper.writeValueAsString(Map.of("destinationId", dest.getId(),
                "departureDate", LocalDate.now().plusDays(5).toString(), "returnDate",
    ...
    List<TripEntity> trips = tripRepository.findAllByUserIdOrderByCreatedAtDesc(user.getId());
    org.junit.jupiter.api.Assertions.assertEquals(1, trips.size());
    org.junit.jupiter.api.Assertions.assertEquals(dest.getId(),
        trips.getFirst().getDestination().getId());
  }

  /**
   * {@code GET /api/trips} must scope results to {@link CurrentUser#id()};
   * another user's trip must not appear in the list.
   */
  @Test
  @DisplayName("GET /api/trips returns only trips for current user")
  void listTrips() throws Exception {
    ...
    mockMvc.perform(get("/api/trips")).andExpect(status().isOk())
        .andExpect(jsonPath("$.success").value(true)).andExpect(jsonPath("$.data.trips").isArray())
        .andExpect(jsonPath("$.data.trips.length()").value(1))
        .andExpect(jsonPath("$.data.trips[0].destinationName").value("ListNice"));
  }

  /**
   * Unknown {@code destinationId} should surface
   * {@link com.univoyage.exception.ResourceNotFoundException} as HTTP 404 with
   * message {@code Destination not found}.
   */
  @Test
  @DisplayName("POST /api/trips returns 404 when destination does not exist")
  void createTripUnknownDestination() throws Exception {
    ...
    mockMvc
        .perform(post("/api/trips").contentType(MediaType.APPLICATION_JSON)
            .content(objectMapper.writeValueAsString(Map.of("destinationId", 999_999L,
                "departureDate", LocalDate.now().plusDays(1).toString(), "returnDate",
  ...
  void deleteTrip() throws Exception {
    ...
    mockMvc.perform(delete("/api/trips/{tripId}", trip.getId())).andExpect(status().isOk())
        .andExpect(jsonPath("$.success").value(true));
  ...
  /**
   * Budget JSON is stored via {@code PUT} and read back with {@code GET}; asserts
   * nested fields in {@code data.budget}.
   */
  @Test
  @DisplayName("PUT and GET /api/trips/{id}/budget round-trip")
  void budgetRoundTrip() throws Exception {
    ...
    mockMvc
        .perform(
            put("/api/trips/{tripId}/budget", trip.getId()).contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(payload)))
        .andExpect(status().isOk()).andExpect(jsonPath("$.success").value(true));

    mockMvc.perform(get("/api/trips/{tripId}/budget", trip.getId())).andExpect(status().isOk())
        .andExpect(jsonPath("$.success").value(true))
        .andExpect(jsonPath("$.data.budget.total").value(1200))
        .andExpect(jsonPath("$.data.budget.currency").value("EUR"));
  }
  ...
  /**
   * Itinerary payload round-trip through PUT/GET; verifies array content survives
   * persistence.
   */
  @Test
  @DisplayName("PUT and GET /api/trips/{id}/itinerary round-trip")
  void itineraryRoundTrip() throws Exception {
    ...
    mockMvc.perform(get("/api/trips/{tripId}/itinerary", trip.getId())).andExpect(status().isOk())
        .andExpect(jsonPath("$.success").value(true))
        .andExpect(jsonPath("$.data.itinerary.days").isArray())
        .andExpect(jsonPath("$.data.itinerary.days[0].day").value(1));
  }
  ...
  /**
   * Non-owner must receive 404 ({@code Trip not found}) and the trip row must
   * remain (no orphan delete).
   */
  @Test
  @DisplayName("DELETE /api/trips/{id} returns 404 when trip belongs to another user")
  void deleteTripNotOwned() throws Exception {
    ...
    mockMvc.perform(delete("/api/trips/{tripId}", trip.getId())).andExpect(status().isNotFound())
        .andExpect(jsonPath("$.error").value("Trip not found"));
  ...
  /**
   * {@link com.univoyage.trip.dto.CreateTripRequest} validation:
   * {@code destinationId} must be non-null.
   */
  @Test
  @DisplayName("POST /api/trips returns 400 when destinationId is null")
  void createTripNullDestinationId() throws Exception {
  ...
  /**
   * Missing {@code departureDate} fails
  ...
  /**
   * Blank {@code departureDate} fails bean validation
   * ({@link jakarta.validation.constraints.NotBlank}).
   */
  @Test
  @DisplayName("POST /api/trips returns 400 when departureDate is blank")
  void createTripBlankDepartureDate() throws Exception {
    ...
    mockMvc
        .perform(post("/api/trips").contentType(MediaType.APPLICATION_JSON)
            .content(objectMapper.writeValueAsString(Map.of("destinationId", dest.getId(),
                "departureDate", "   ", "returnDate", LocalDate.now().plusDays(3).toString()))))
        .andExpect(status().isBadRequest()).andExpect(jsonPath("$.success").value(false));
  }

  /**
   * Blank {@code returnDate} fails bean validation
   * ({@link jakarta.validation.constraints.NotBlank}).
   */
  @Test
  @DisplayName("POST /api/trips returns 400 when returnDate is blank")
  ...
  /**
   * Non-ISO date strings cause {@link java.time.format.DateTimeParseException} in
   * the service; {@link com.univoyage.exception.GlobalExceptionHandler} maps it
   * to HTTP 500 with generic JSON (no message leak).
   */
  @Test
  @DisplayName("POST /api/trips returns 500 JSON when date format is invalid")
  void createTripInvalidDateFormat() throws Exception {
    ...
    mockMvc.perform(post("/api/trips").contentType(MediaType.APPLICATION_JSON)
        .content(objectMapper.writeValueAsString(Map.of("destinationId", dest.getId(),
            "departureDate", "not-a-date", "returnDate", LocalDate.now().plusDays(3).toString()))))
        .andExpect(status().isInternalServerError()).andExpect(jsonPath("$.success").value(false))
        .andExpect(jsonPath("$.error").value("An unexpected error occurred."));
  }

  /**
   * Database {@code trips_dates_chk} rejects
   * {@code return_date < departure_date}; the persistence error is mapped to HTTP
   * 500 with generic JSON by
   * {@link com.univoyage.exception.GlobalExceptionHandler} (no message leak).
   */
  @Test
  @DisplayName("POST /api/trips returns 500 JSON when return date is before departure date")
  void createTripReturnBeforeDeparture() throws Exception {
    ...
    mockMvc
        .perform(post("/api/trips").contentType(MediaType.APPLICATION_JSON)
            .content(objectMapper.writeValueAsString(Map.of("destinationId", dest.getId(),
                "departureDate", d0.plusDays(5).toString(), "returnDate", d0.toString()))))
        .andExpect(status().isInternalServerError()).andExpect(jsonPath("$.success").value(false))
        .andExpect(jsonPath("$.error").value("An unexpected error occurred."));
  }
  ...
  void deleteTripNotFound() throws Exception {
    ...
    mockMvc.perform(delete("/api/trips/{tripId}", 999_998L)).andExpect(status().isNotFound())
        .andExpect(jsonPath("$.error").value("Trip not found"));
  }

  /**
   * Budget GET without prior PUT returns empty JSON object under
   * {@code data.budget}.
   */
  @Test
  @DisplayName("GET /api/trips/{id}/budget returns empty object when no budget saved")
  ...
  /**
   * Non-owner cannot read another user's budget (same {@code Trip not found}
   * semantics as other trip APIs).
   */
  @Test
  @DisplayName("GET /api/trips/{id}/budget returns 404 when trip is not owned")
  void getBudgetNotOwned() throws Exception {
    ...
    mockMvc.perform(get("/api/trips/{tripId}/budget", trip.getId()))
        .andExpect(status().isNotFound()).andExpect(jsonPath("$.error").value("Trip not found"));
  }
  ...
  void putItineraryNotOwned() throws Exception {
    ...
    mockMvc
        .perform(put("/api/trips/{tripId}/itinerary", trip.getId())
            .contentType(MediaType.APPLICATION_JSON)
            .content(objectMapper.writeValueAsString(Map.of("days", List.of()))))
        .andExpect(status().isNotFound()).andExpect(jsonPath("$.error").value("Trip not found"));
  }
  ...
  /**
   * Itinerary GET without prior PUT returns empty JSON object under
   * {@code data.itinerary}, matching budget semantics.
   */
  @Test
  @DisplayName("GET /api/trips/{id}/itinerary returns empty object when no itinerary saved")
  void getItineraryWhenNoneStored() throws Exception {
  ...
  void accommodationRoundTrip() throws Exception {
    ...
    Map<String, String> req = Map.of("accommodationName", "Test Hotel", "accommodationAddress",
        "1 Main St", "accommodationPhone", "+1000000000");

    mockMvc
        .perform(put("/api/trips/{tripId}/accommodation", trip.getId())
  ...
  void putAccommodationNotOwned() throws Exception {
    ...
    mockMvc
        .perform(put("/api/trips/{tripId}/accommodation", trip.getId())
            .contentType(MediaType.APPLICATION_JSON)
            .content(objectMapper.writeValueAsString(Map.of("accommodationName", "H",
                "accommodationAddress", "A", "accommodationPhone", "P"))))
        .andExpect(status().isNotFound()).andExpect(jsonPath("$.error").value("Trip not found"));
  }
  ...
  void putBudgetMalformedJson_returns400ConsistentError() throws Exception {
    ...
    mockMvc
        .perform(put("/api/trips/{tripId}/budget", trip.getId())
            .contentType(MediaType.APPLICATION_JSON).content("{\"total\": 123"))
        .andExpect(status().isBadRequest()).andExpect(jsonPath("$.success").value(false))
        .andExpect(jsonPath("$.error").isString());
  }
  ...
  void putItineraryMalformedJson_returns400ConsistentError() throws Exception {
    ...
    mockMvc
        .perform(put("/api/trips/{tripId}/itinerary", trip.getId())
            .contentType(MediaType.APPLICATION_JSON).content("{\"days\": [}"))
        .andExpect(status().isBadRequest()).andExpect(jsonPath("$.success").value(false))
        .andExpect(jsonPath("$.error").isString());
  }
  ...
  void putAccommodationMissingRequiredFields_returns400Validation() throws Exception {
    ...
    mockMvc
        .perform(put("/api/trips/{tripId}/accommodation", trip.getId())
            .contentType(MediaType.APPLICATION_JSON)
            .content(objectMapper.writeValueAsString(Map.of("accommodationPhone", "+359000000"))))
        .andExpect(status().isBadRequest()).andExpect(jsonPath("$.success").value(false))
        .andExpect(jsonPath("$.error").value("Validation failed for the request payload."))
        .andExpect(jsonPath("$.data.accommodationName").exists())
        .andExpect(jsonPath("$.data.accommodationAddress").exists());
  ...
  void putAccommodationMalformedJson_returns400ConsistentError() throws Exception {
    ...
    mockMvc
        .perform(put("/api/trips/{tripId}/accommodation", trip.getId())
            .contentType(MediaType.APPLICATION_JSON).content("{\"accommodationName\":\"Hotel\","))
        .andExpect(status().isBadRequest()).andExpect(jsonPath("$.success").value(false))
        .andExpect(jsonPath("$.error").isString());
  }

  /**
   * Accommodation GET without prior PUT returns empty JSON object under
   * {@code data.accommodation}, matching budget semantics.
   */
  @Test
  @DisplayName("GET /api/trips/{id}/accommodation returns empty object when no accommodation saved")
  void getAccommodationWhenNoneStored() throws Exception {
  ...
  /**
   * Creates a destination with unique name and location to satisfy the DB unique
   * constraint.
   */
  private DestinationEntity saveDestination(String name, Country country) {
    return destinationRepository.save(DestinationEntity.builder().name(name).location(name)
        .continent("Europe").country(country).imageUrl("https://example.com/x.jpg").imageAlt(name)
