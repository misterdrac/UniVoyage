...
/**
 * Full-stack integration tests for {@link WeatherController} with
 * {@link WeatherService} mocked.
 */
@SpringBootTest
...
class WeatherControllerIntegrationTest {
  ...
  void weatherCurrentRequiresLocation() throws Exception {
    mockMvc.perform(get("/api/weather/current")).andExpect(status().isBadRequest())
  ...
  void weatherCurrentByCoordinates() throws Exception {
    ...
    mockMvc.perform(get("/api/weather/current").param("lat", "45.5").param("lon", "9.2"))
  ...
  void weatherCurrentByCity() throws Exception {
    ...
    mockMvc.perform(get("/api/weather/current").param("city", "Paris").param("country", "FR"))
        .andExpect(status().isOk()).andExpect(jsonPath("$.data.weather.city").value("Paris"))
  ...
  void weatherForecastOk() throws Exception {
    ...
    mockMvc.perform(get("/api/weather/forecast").param("city", "Oslo")).andExpect(status().isOk())
