...
/**
 * Full-stack integration tests for {@link PlacesController} with
 * {@link GeoapifyService} mocked.
 */
@SpringBootTest
...
class PlacesControllerIntegrationTest {
  ...
  void placesSearchOk() throws Exception {
    ...
    mockMvc.perform(get("/api/places/search").param("city", "Lyon").param("limit", "10"))
  ...
  void placesSearchCapsLimit() throws Exception {
    ...
    mockMvc.perform(get("/api/places/search").param("city", "Zurich").param("limit", "999"))
  ...
  void placesSearchBadGateway() throws Exception {
    ...
    mockMvc.perform(get("/api/places/search").param("city", "Y")).andExpect(status().isBadGateway())
