...
class TripTravellerRatingIntegrationTest {
  ...
  void shouldAcceptRatingAfterReturnDate() throws Exception {
    ...
    mockMvc
        .perform(
            post("/api/trips/{tripId}/rating", trip.getId()).contentType(MediaType.APPLICATION_JSON)
    ...
    mockMvc.perform(get("/api/trips/{tripId}/rating", trip.getId())).andExpect(status().isOk())
        .andExpect(jsonPath("$.data.rating.stars").value(4));

    DestinationEntity beforeApprove = destinationRepository.findById(destination.getId())
        .orElseThrow();
  ...
  void shouldAcceptRatingWhenReturnDateIsToday() throws Exception {
    ...
    mockMvc
        .perform(post("/api/trips/{tripId}/rating", trip.getId())
            .contentType(MediaType.APPLICATION_JSON).content("{\"stars\":5}"))
        .andExpect(status().isOk()).andExpect(jsonPath("$.data.rating.stars").value(5))
        .andExpect(jsonPath("$.data.rating.moderationStatus").value("APPROVED"));
  }
  ...
  void shouldAcceptRatingWhenCompletedDespiteFutureReturn() throws Exception {
    ...
    mockMvc
        .perform(post("/api/trips/{tripId}/rating", trip.getId())
            .contentType(MediaType.APPLICATION_JSON).content("{\"stars\":3}"))
        .andExpect(status().isOk()).andExpect(jsonPath("$.data.rating.stars").value(3))
        .andExpect(jsonPath("$.data.rating.moderationStatus").value("APPROVED"));
  }
  ...
  void shouldUpdateRatingOnSecondSubmit() throws Exception {
    ...
    mockMvc
        .perform(post("/api/trips/{tripId}/rating", trip.getId())
            .contentType(MediaType.APPLICATION_JSON).content("{\"stars\":2,\"comment\":\"Meh\"}"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.data.rating.moderationStatus").value("PENDING"));

    approveReview(trip.getId(), user.getId());
    Assertions.assertThat(
        destinationRepository.findById(destination.getId()).orElseThrow().getTravellerRatingCount())
        .isEqualTo(1);
  ...
  void shouldNormalizeBlankCommentToNull() throws Exception {
    ...
    mockMvc
        ...
        .andExpect(status().isOk()).andExpect(jsonPath("$.data.rating.comment").value(nullValue()))
        .andExpect(jsonPath("$.data.rating.moderationStatus").value("APPROVED"));
  ...
  void postRating_returns404WhenTripMissing() throws Exception {
    ...
    mockMvc
        .perform(post("/api/trips/{tripId}/rating", 9_999_999_999L)
            .contentType(MediaType.APPLICATION_JSON).content("{\"stars\":5}"))
        .andExpect(status().isNotFound()).andExpect(jsonPath("$.error").value("Trip not found"));
  }
  ...
  void shouldRejectStarsTooLow() throws Exception {
    ...
    mockMvc
        .perform(post("/api/trips/{tripId}/rating", trip.getId())
            .contentType(MediaType.APPLICATION_JSON).content("{\"stars\":0}"))
        .andExpect(status().isBadRequest()).andExpect(jsonPath("$.success").value(false))
  ...
  void shouldRejectCommentTooLong() throws Exception {
    ...
    String body = objectMapper.writeValueAsString(Map.of("stars", 5, "comment", "c".repeat(2001)));
  ...
  void shouldAcceptStarsWithoutComment() throws Exception {
    ...
    mockMvc
        ...
        .andExpect(status().isOk()).andExpect(jsonPath("$.data.rating.stars").value(1))
        .andExpect(jsonPath("$.data.rating.comment").value(nullValue()))
        .andExpect(jsonPath("$.data.rating.moderationStatus").value("APPROVED"));
  }
  ...
  void destinationAggregate_reflectsMultipleTrips() throws Exception {
    ...
    mockMvc
        .perform(post("/api/trips/{tripId}/rating", tripA.getId())
    ...
    mockMvc
        .perform(post("/api/trips/{tripId}/rating", tripB.getId())
            .contentType(MediaType.APPLICATION_JSON).content("{\"stars\":2}"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.data.rating.moderationStatus").value("APPROVED"));
  ...
  private void approveReview(long tripId, long userId) throws Exception {
    long ratingId = tripTravellerRatingRepository.findByTripIdAndUserId(tripId, userId)
        .orElseThrow().getId();
  ...
  private TripEntity saveFutureTrip(Long userId, DestinationEntity destination) {
    LocalDate today = LocalDate.now();
    return tripRepository.save(TripEntity.builder().userId(userId).destination(destination)
        .departureDate(today.plusDays(1)).returnDate(today.plusDays(7)).status("planned")
        .createdAt(Instant.now()).build());
  }
  ...
  private TripEntity saveTripReturnToday(Long userId, DestinationEntity destination) {
    LocalDate today = LocalDate.now();
    return tripRepository.save(TripEntity.builder().userId(userId).destination(destination)
        .departureDate(today.minusDays(5)).returnDate(today).status("ongoing")
        .createdAt(Instant.now()).build());
  }
  ...
  private TripEntity saveCompletedTripFutureReturn(Long userId, DestinationEntity destination) {
    LocalDate today = LocalDate.now();
    return tripRepository.save(TripEntity.builder().userId(userId).destination(destination)
        .departureDate(today.minusDays(1)).returnDate(today.plusDays(30)).status("completed")
        .createdAt(Instant.now()).build());
  }
}
