 ...
 * <p>
 * Hits the real web layer and persistence;
 * {@link com.univoyage.currency.CurrencyRateService} and {@link CurrentUser}
 * are mocked to avoid external FX calls and to control the authenticated user
 * id. Servlet filters are disabled so JWT/CSRF do not affect assertions.
 * </p>
 */
@SpringBootTest
...
class TripControllerCurrencyIntegrationTest {
  ...
  /**
   * Happy path: user's home country currency is the FX base; destination
   * country's currency and mocked rate appear under {@code data.currency}.
   */
  @Test
  ...
  void shouldReturnCurrencyPayloadWhenTripBelongsToCurrentUser() throws Exception {
    ...
    mockMvc.perform(get("/api/trips/{tripId}/currency", trip.getId())).andExpect(status().isOk())
  ...
  /**
   * When {@link UserEntity#getCountry()} is null, base currency defaults to EUR
   * and the rate call uses EUR as the from-currency.
   */
  @Test
  ...
  /**
   * Blank destination country currency code yields {@link IllegalStateException},
   * mapped by {@link com.univoyage.exception.GlobalExceptionHandler} to HTTP 500
  ...
  void shouldReturn500WhenDestinationCurrencyIsNotConfigured() throws Exception {
    ...
    DestinationEntity destination = saveDestination("Mystery", "Unknown", "Europe",
  ...
  /**
   * Missing trip id returns HTTP 404 ({@code Trip not found}) via global
   * exception handling.
   */
  @Test
  ...
  void shouldReturn404WhenTripDoesNotExist() throws Exception {
    ...
    mockMvc.perform(get("/api/trips/{tripId}/currency", 999999L)).andExpect(status().isNotFound())
        .andExpect(jsonPath("$.success").value(false))
        .andExpect(jsonPath("$.error").value("Trip not found"));
  }

  /**
   * Ownership mismatch is intentionally surfaced as {@code Trip not found} (no
  ...
  /**
   * User's home country currency (e.g. USD) becomes the FX base when present;
   * rate service is called with that base code.
   */
  @Test
  ...
  /**
   * Unchecked failures from {@link CurrencyRateService} propagate (HTTP 500)
   * unless mapped elsewhere.
   */
  @Test
  ...
  /**
   * Persists a {@link Country} row for test isolation (use rare ISO codes when
   * needed).
   */
  private Country saveCountry(String isoCode, String countryName, String currencyCode,
      String currencyName) {
    Country country = Country.builder().isoCode(isoCode).countryName(countryName)
  ...
  private UserEntity saveUser(String email, Country country) {
    UserEntity user = UserEntity.builder().name("Test").surname("User").email(email)
  ...
  private DestinationEntity saveDestination(String name, String location, String continent,
      Country country) {
    DestinationEntity destination = DestinationEntity.builder().name(name).location(location)
        .continent(continent).country(country).imageUrl("https://example.com/image.jpg")
        .imageAlt("Test image").overview("Test overview").budgetPerDay(100).whyVisit("Test reason")
  ...
  private TripEntity saveTrip(Long userId, DestinationEntity destination) {
    TripEntity trip = TripEntity.builder().userId(userId).destination(destination)
        .departureDate(LocalDate.now().plusDays(10)).returnDate(LocalDate.now().plusDays(15))
