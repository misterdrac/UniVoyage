...
class AuthCookieContractIntegrationTest {
    ...
    MvcResult result = mockMvc
        .perform(post("/api/auth/login").contentType(MediaType.APPLICATION_JSON)
            .content(objectMapper.writeValueAsString(
                Map.of("email", "cookie-login@example.com", "password", "Str0ng!Pass"))))
  ...
  void refresh_cookieContract() throws Exception {
    ...
    MvcResult loginResult = mockMvc
        .perform(post("/api/auth/login").contentType(MediaType.APPLICATION_JSON)
            .content(objectMapper.writeValueAsString(
                Map.of("email", "cookie-refresh@example.com", "password", "Str0ng!Pass"))))
        .andExpect(status().isOk()).andReturn();
  ...
  private void registerUser(String email, String countryCode) throws Exception {
    mockMvc.perform(post("/api/auth/register").contentType(MediaType.APPLICATION_JSON)
        .content(objectMapper.writeValueAsString(Map.of("email", email, "password", "Str0ng!Pass",
            "name", "Cookie", "surname", "Contract", "countryCode", countryCode))))
  ...
  private void assertCookieContract(List<String> setCookies, String cookieName,
      boolean expectedHttpOnly) {
    String header = setCookies.stream().filter(v -> v.startsWith(cookieName + "=")).findFirst()
