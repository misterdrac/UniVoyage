...
@SpringBootTest(properties = {"app.auth.login.ip-max-attempts=2", "app.auth.login.ip-window=PT1H"})
...
class AuthLoginRateLimitIntegrationTest {
  ...
  void loginRateLimitedByIp() throws Exception {
    String body = objectMapper
        .writeValueAsString(Map.of("email", "anyone@example.com", "password", "irrelevant"));
  ...
  void rateLimitUsesXForwardedForWhenPresent() throws Exception {
    String body = objectMapper
        .writeValueAsString(Map.of("email", "x@example.com", "password", "x"));

    for (int i = 0; i < 2; i++) {
      mockMvc
          .perform(post("/api/auth/login").header("X-Forwarded-For", "203.0.113.10")
    ...
    mockMvc
        .perform(post("/api/auth/login").header("X-Forwarded-For", "203.0.113.20")
            .contentType(MediaType.APPLICATION_JSON).content(body))
        .andExpect(status().isUnauthorized());
  }
}
