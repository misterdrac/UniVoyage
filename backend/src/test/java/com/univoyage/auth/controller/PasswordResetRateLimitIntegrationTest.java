...
@SpringBootTest(properties = {
    "app.auth.password-reset.forgot-ip-max-attempts=3",
...
class PasswordResetRateLimitIntegrationTest {
  ...
  void setup() {
    Country country = countryRepository.findByIsoCode("HR")
        .orElseGet(() -> countryRepository.save(
            Country.builder().isoCode("HR").countryName("Croatia")
                .currencyCode("EUR").currencyName("Euro").build()));

    userRepository.save(UserEntity.builder()
        .email("resetrl@test.com").name("Reset").surname("RL")
  ...
  void forgotRateLimitedByIp() throws Exception {
    for (int i = 0; i < 3; i++) {
      mockMvc.perform(post("/api/auth/password/forgot")
              ...
              .content(objectMapper.writeValueAsString(
                  Map.of("email", "resetrl-ip" + i + "@test.com"))))
    ...
    mockMvc.perform(post("/api/auth/password/forgot")
            ...
            .contentType(MediaType.APPLICATION_JSON)
            .content(objectMapper.writeValueAsString(
                Map.of("email", "resetrl-ip99@test.com"))))
  ...
  void resetSubmitRateLimitedByIp() throws Exception {
    ...
    mockMvc.perform(post("/api/auth/password/reset")
            ...
            .contentType(MediaType.APPLICATION_JSON)
            .content(objectMapper.writeValueAsString(
                Map.of("token", "fake-token-x", "newPassword", "NewPass123!"))))
        .andExpect(status().isTooManyRequests())
        .andExpect(header().exists("Retry-After"));
  }
}
