...
/**
 * Integration tests for {@link GoogleOAuthController} with
 * {@link GoogleOAuthService} mocked so no Google credentials or outbound HTTP
 * are required.
 */
@SpringBootTest
...
class GoogleOAuthControllerIntegrationTest {
  ...
  void googleAuth_redirects() throws Exception {
    ...
    mockMvc.perform(get("/api/auth/google")).andExpect(status().isFound())
  ...
  void googleAuth_serviceThrows_returns500() throws Exception {
    ...
    mockMvc.perform(get("/api/auth/google")).andExpect(status().isInternalServerError());
  }
  ...
  void callback_missingCode_returns400() throws Exception {
    mockMvc
        .perform(
            post("/api/auth/google/callback").contentType(MediaType.APPLICATION_JSON).content("{}"))
  ...
  void callback_success_setsCookiesAndBody() throws Exception {
    UserEntity persisted = userRepository
    ...
    MvcResult result = mockMvc
        ...
        .andExpect(jsonPath("$.data.csrfToken").value("test-csrf")).andReturn();

    List<String> setCookies = result.getResponse().getHeaders(HttpHeaders.SET_COOKIE);
    assertThat(setCookies).isNotEmpty();
    assertThat(
        setCookies.stream().anyMatch(c -> c.startsWith(CookieUtils.JWT_COOKIE_NAME + "=test-jwt")))
        .as("JWT cookie").isTrue();
    assertThat(setCookies.stream()
        .anyMatch(c -> c.startsWith(CookieUtils.CSRF_COOKIE_NAME + "=test-csrf"))).as("CSRF cookie")
        .isTrue();
    assertThat(setCookies.stream()
        .anyMatch(c -> c.startsWith(CookieUtils.REFRESH_TOKEN_COOKIE_NAME + "=")))
        .as("Refresh cookie").isTrue();
  }
}
