...
class SecurityIntegrationTest {
  ...
  void adminDestinations_unauthenticated() throws Exception {
    mockMvc.perform(get("/api/admin/destinations")).andExpect(status().isUnauthorized())
        .andExpect(jsonPath("$.success").value(false))
        .andExpect(jsonPath("$.error").value(UNAUTHORIZED_MESSAGE));
  }
  ...
  void createTrip_unauthenticated() throws Exception {
    mockMvc.perform(post("/api/trips").contentType(MediaType.APPLICATION_JSON).content("{}"))
  ...
  void tripBudget_unauthenticated() throws Exception {
    mockMvc.perform(get("/api/trips/1/budget")).andExpect(status().isUnauthorized())
        .andExpect(jsonPath("$.success").value(false))
        .andExpect(jsonPath("$.error").value(UNAUTHORIZED_MESSAGE));
  }
  ...
  void aiItinerary_unauthenticated() throws Exception {
    mockMvc.perform(post("/api/ai/itinerary").contentType(MediaType.APPLICATION_JSON).content("{}"))
        .andExpect(status().isUnauthorized()).andExpect(jsonPath("$.success").value(false))
        .andExpect(jsonPath("$.error").value(UNAUTHORIZED_MESSAGE));
  }
  ...
  void adminPatchRole_forbiddenForPlainUser() throws Exception {
    ...
    mockMvc
        .perform(patch("/api/admin/users/1/role").with(securityContextFor(user))
            .contentType(MediaType.APPLICATION_JSON).content("{\"role\":\"ADMIN\"}"))
        .andExpect(status().isForbidden()).andExpect(jsonPath("$.success").value(false))
        .andExpect(jsonPath("$.error").value(FORBIDDEN_MESSAGE));
  }

  // --- Public routes must not be rejected as unauthenticated (no 401 from
  // security) -

  @Test
  @DisplayName("GET /api/destinations is publicly accessible (not 401)")
  void destinations_publiclyAccessible() throws Exception {
    mockMvc.perform(get("/api/destinations")).andExpect(status().is2xxSuccessful());
  }

  @Test
  @DisplayName("GET /api/heatmap is publicly accessible (not 401)")
  void heatmap_publiclyAccessible() throws Exception {
    mockMvc.perform(get("/api/heatmap")).andExpect(status().is2xxSuccessful());
  }

  /**
   * Quiz is permitAll under /api/quiz/**; there is no GET handler on /api/quiz.
   * Assert unauthenticated POST hits the app (validation/service), not Spring
   * Security 401.
   */
  @Test
  ...
  void quiz_recommend_notUnauthorizedWithoutToken() throws Exception {
    mockMvc
        .perform(post("/api/quiz/recommend").contentType(MediaType.APPLICATION_JSON).content("{}"))
