...
class DestinationReviewIntegrationTest {
  ...
  void listReviews_showsApprovedOnly() throws Exception {
    ...
    mockMvc.perform(get("/api/destinations/{id}/reviews", destination.getId()))
        .andExpect(status().isOk()).andExpect(jsonPath("$.data.reviews.content", hasSize(0)));

    long ratingId = tripTravellerRatingRepository.findByTripIdAndUserId(trip.getId(), user.getId())
        .orElseThrow().getId();
    mockMvc.perform(post("/api/admin/reviews/{id}/approve", ratingId)).andExpect(status().isOk());

    mockMvc.perform(get("/api/destinations/{id}/reviews", destination.getId()))
        .andExpect(status().isOk()).andExpect(jsonPath("$.data.reviews.content", hasSize(1)))
  ...
  void listReviews_hidesRejected() throws Exception {
    ...
    mockMvc.perform(post("/api/trips/{tripId}/rating", trip.getId())
        .contentType(MediaType.APPLICATION_JSON).content("{\"stars\":1,\"comment\":\"spam spam\"}"))
        .andExpect(status().isOk());

    long ratingId = tripTravellerRatingRepository.findByTripIdAndUserId(trip.getId(), user.getId())
        .orElseThrow().getId();
    mockMvc.perform(post("/api/admin/reviews/{id}/reject", ratingId)).andExpect(status().isOk());
