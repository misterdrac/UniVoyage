...
/**
 * Integration tests for issue #203: normalized JSON errors from
 * {@link GlobalExceptionHandler} for common failure modes (validation, bad
 * types, missing params). {@link IllegalArgumentException} mapping is covered
 * by {@link GlobalExceptionHandlerIllegalArgumentWebMvcTest}.
 */
@SpringBootTest
...
class GlobalExceptionHandlerIntegrationTest {
  ...
  void typeMismatch_mapsTo400Json() throws Exception {
    ...
    mockMvc.perform(get("/api/trips/not-a-number/budget")).andExpect(status().isBadRequest())
  ...
  void malformedJson_mapsTo400Json() throws Exception {
    ...
    mockMvc
        .perform(
            put("/api/trips/1/budget").contentType(MediaType.APPLICATION_JSON).content("{\"x\":"))
        .andExpect(status().isBadRequest()).andExpect(jsonPath("$.success").value(false))
  ...
  void notFound_mapsTo404Json() throws Exception {
    ...
    mockMvc.perform(get("/api/trips/999999999/budget")).andExpect(status().isNotFound())
        .andExpect(jsonPath("$.success").value(false))
