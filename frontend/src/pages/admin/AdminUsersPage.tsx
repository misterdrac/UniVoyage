...
const AdminUsersPage: React.FC = () => {
  ...
  const handleSaveChanges = async () => {
    ...
    setIsSaving(true);
    try {
      const updatedUser = await apiService.updateUserRole(
        selectedUser.id,
        editRole,
      );
                      ...
                      users.map((user) => (
                        ...
                        >
                            ...
                            <span
                              ...
                              style={{
                                backgroundColor:
                                  user.role === "ADMIN" ||
                                  user.role === "HEAD_ADMIN"
                                    ? "var(--admin-badge-admin-bg)"
                                    : "var(--admin-badge-user-bg)",
                                color:
                                  user.role === "ADMIN" ||
                                  user.role === "HEAD_ADMIN"
                                    ? "var(--admin-badge-admin-text)"
                                    : "var(--admin-badge-user-text)",
                              }}
                            >
                              {user.role}
                            </span>
                          ...
                          <td className="px-4 py-3 text-sm text-muted-foreground whitespace-nowrap text-center">
                            {user.dateOfRegister
                              ? formatDateShort(user.dateOfRegister, "en-US", {
                                  year: "numeric",
                                  month: "short",
                                  day: "numeric",
                                })
              ...
              <div className="px-6 pb-6 overflow-y-auto dropdown-scrollbar flex-1 min-h-0">
                <div className="space-y-5">
                  ...
                  <div className="space-y-2">
                    ...
                    {currentUser?.role === "ADMIN" &&
                      ...
                      (selectedUser.role === "ADMIN" ||
                        selectedUser.role === "HEAD_ADMIN") && (
                        <p className="text-xs text-muted-foreground mt-2">
                          Admins cannot modify other admin accounts. Contact the
                          head admin to change the role.
                        </p>
                      )}
                  </div>
                  ...
                  <Button
                    ...
                    style={{
                      background:
                        "linear-gradient(to right, var(--admin-users-gradient-start), var(--admin-users-gradient-end))",
                    }}
                  >
                    {isSaving ? (
                      <>
