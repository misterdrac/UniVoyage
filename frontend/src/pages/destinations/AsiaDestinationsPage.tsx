...
const AsiaDestinationsPage = () => {
  ...
  const getRandomDestinations = (count: number) => {
    const shuffled = [...asiaDestinations].sort(() => 0.5 - Math.random());
  ...
  const filteredDestinations = useMemo(() => {
    if (selectedCountry) {
      return asiaDestinations.filter(dest => dest.location === selectedCountry.label);
    }
    return getRandomDestinations(6);
  }, [selectedCountry, asiaDestinations]);
        ...
        <div className="relative z-40 bg-card/50 backdrop-blur rounded-xl border border-border/50 p-6 sm:p-8 mb-12 sm:mb-16">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8 mb-6">
            <div className="flex items-center">
              <h1 className="text-3xl sm:text-4xl lg:text-5xl font-bold text-foreground">Discover Asia</h1>
            </div>
            <div className="flex items-center">
          ...
          <div className={isAnimating ? "animate-pulse" : ""}>
            <DestinationPicker continent="Asia" />
          </div>
        </div>
