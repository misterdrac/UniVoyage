...
export interface HotelsApi {
  ...
  searchHotels(
    city: string,
    limit?: number,
  ): Promise<{ success: boolean; hotels?: Hotel[]; error?: string }>;
...
export const hotelsApi: {
  [K in keyof HotelsApi]: (
    this: ApiClient,
    ...args: Parameters<HotelsApi[K]>
  ) => ReturnType<HotelsApi[K]>;
} = {
  async searchHotels(
    this: ApiClient,
    city: string,
    ...
    try {
      ...
      if (
        errorMsg.includes("NOTHING FOUND") ||
        errorMsg.includes("Nothing found")
      ) {
        return {
          success: false,
      ...
      if (
        errorMessage.includes("NOTHING FOUND") ||
        errorMessage.includes("Nothing found")
      ) {
        return {
          success: false,
