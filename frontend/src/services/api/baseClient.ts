...
export class ApiClient {
  ...
  public adaptUserDto(user?: BackendUserDto): User | undefined {
    ...
    const visitedCountries: VisitedCountryDto[] = (
      user.visitedCountries ?? []
    ).map((vc) => ({
      ...vc,
      dateOfVisit:
        typeof vc.dateOfVisit === "string"
          ? vc.dateOfVisit
          : vc.dateOfVisit
            ? new Date(vc.dateOfVisit).toISOString()
  ...
  public adaptAuthPayload(
    payload?: AuthResponse<BackendUserDto>,
  ): AuthResponse<User> {
