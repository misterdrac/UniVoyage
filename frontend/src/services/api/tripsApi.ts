...
export interface TripsApi {
  ...
  getTripBudget(
    tripId: number,
  ): Promise<{ success: boolean; budget?: TripBudgetPayload; error?: string }>;
  ...
  getTripCurrency(
    tripId: number,
  ): Promise<{ success: boolean; currency?: TripCurrencyInfo; error?: string }>;
  ...
  getTripRating(
    tripId: number,
  ): Promise<{ success: boolean; rating?: TripRating | null; error?: string }>;
  ...
  submitTripRating(
    ...
    stars: number,
    comment?: string,
  ): Promise<{ success: boolean; rating?: TripRating; error?: string }>;
}
...
export const tripsApi: {
  [K in keyof TripsApi]: (
    this: ApiClient,
    ...args: Parameters<TripsApi[K]>
  ) => ReturnType<TripsApi[K]>;
} = {
  ...
  async getTripBudget(this: ApiClient, tripId) {
    try {
      ...
      return {
        success: false,
        error: res.error ?? "Failed to load trip budget",
      };
    } catch (err: any) {
      return {
        success: false,
        error: err?.message ?? "Failed to load trip budget",
      };
    }
  },
  ...
  async saveTripBudget(this: ApiClient, tripId, payload) {
    try {
      ...
      return {
        success: false,
        error: res.error ?? "Failed to save trip budget",
      };
    } catch (err: any) {
      return {
        success: false,
        error: err?.message ?? "Failed to save trip budget",
      };
    }
  },
  ...
  async getTripAccommodation(this: ApiClient, tripId) {
    try {
      ...
      return {
        success: false,
        error: res.error ?? "Failed to load accommodation",
      };
    } catch (err: any) {
      return {
        success: false,
        error: err?.message ?? "Failed to load accommodation",
      };
    }
  },
  ...
  async saveTripAccommodation(this: ApiClient, tripId, data) {
    try {
      ...
      return {
        success: false,
        error: res.error ?? "Failed to save accommodation",
      };
    } catch (err: any) {
      return {
        success: false,
        error: err?.message ?? "Failed to save accommodation",
      };
    }
  },
  ...
  async getTripCurrency(this: ApiClient, tripId) {
    try {
      ...
      return {
        success: false,
        error: err?.message ?? "Failed to load currency",
      };
    }
  },
...
export const itineraryApi: {
  [K in keyof ItineraryApi]: (
    this: ApiClient,
    ...args: Parameters<ItineraryApi[K]>
  ) => ReturnType<ItineraryApi[K]>;
} = {
  async getTripItinerary(this: ApiClient, tripId) {
    try {
      ...
      return {
        success: false,
        error: err?.message ?? "Failed to load itinerary",
      };
    }
  },
