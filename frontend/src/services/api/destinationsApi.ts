...
export interface DestinationsApi {
  ...
  getDestinationReviews(
    ...
    success: boolean;
    reviews?: DestinationReviewsPage;
    error?: string;
  }>;
}
 ...
 */
function mapBackendDestination(
  backendDest: BackendDestinationResponse,
): Destination {
  return {
    id: backendDest.id,
    ...
    averageRating:
      backendDest.averageRating !== undefined &&
      backendDest.averageRating !== null
        ? Number(backendDest.averageRating)
        : undefined,
    travellerRatingAverage:
      backendDest.travellerRatingAverage !== undefined &&
      backendDest.travellerRatingAverage !== null
...
export const destinationsApi: {
  ...
  async getDestinations(this: ApiClient) {
    try {
      ...
      return {
        success: false,
        error: res.error ?? "Failed to fetch destinations",
      };
    } catch (err: any) {
      return {
        success: false,
  ...
  async searchDestinations(this: ApiClient, query: string) {
    try {
      ...
      return {
        success: false,
        error: res.error ?? "Failed to search destinations",
      };
    } catch (err: any) {
      return {
        success: false,
        error: err?.message ?? "Failed to search destinations",
      };
    }
  },
