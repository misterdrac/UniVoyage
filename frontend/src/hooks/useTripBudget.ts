...
const DEFAULT_CATEGORIES: {
...
}[] = [
  {
    value: "accommodation",
    label: "Accommodation",
    suggestion: "Hotels, hostels, rentals, resort fees",
  },
  {
    value: "transportation",
    label: "Transportation",
    suggestion: "Flights, trains, rideshares, local transit",
  },
  {
    value: "food",
    label: "Food & Dining",
    suggestion: "Restaurants, cafes, groceries, snacks",
  },
  {
    value: "activities",
    label: "Activities",
    suggestion: "Tours, tickets, experiences, nightlife",
  },
  {
    value: "shopping",
    label: "Shopping & Gifts",
    suggestion: "Souvenirs, clothing, gifts",
  },
  {
    value: "misc",
    label: "Miscellaneous",
    suggestion: "Insurance, tips, emergency funds",
  },
];
...
const normalizeExpenses = (rawExpenses: any[] = []): TripBudgetExpense[] =>
  rawExpenses
    ...
    .filter(
      (expense) => typeof expense.id === "string" && expense.id.length > 0,
    );

const normalizeBudgetPayload = (
  data?: TripBudgetPayload | null,
): TripBudgetPayload => {
  ...
  return {
    ...
    totalBudget: Math.min(
      Math.max(Number(data.totalBudget) || 0, 0),
      MAX_TOTAL_BUDGET,
    ),
  };
};
...
export const useTripBudget = (tripId: number | null | undefined) => {
  ...
  const [allocations, setAllocations] =
    useState<Record<BudgetCategoryValue, number>>(emptyAllocations());
  ...
  const updateAllocation = useCallback(
    (category: BudgetCategoryValue, amount: number) => {
      setAllocations((prev) => ({
        ...prev,
  ...
  const updateAllocations = useCallback(
    (values: Record<BudgetCategoryValue, number>) => {
    ...
    setTotalBudget((prev) => {
      if (normalized < prev) {
        ...
        setAllocations((current) => {
          const totalAllocated = Object.values(current).reduce(
            (sum, value) => sum + value,
            0,
          );
          ...
          const scale =
            normalized === 0 || totalAllocated === 0
              ? 0
              : normalized / totalAllocated;
  ...
  const updateExpense = useCallback(
    (id: string, updates: Partial<Omit<TripBudgetExpense, "id">>) => {
      setExpenses((prev) =>
        prev.map((expense) =>
          expense.id === id
            ? {
                ...
                amount:
                  updates.amount !== undefined
                    ? parseFloat(Number(updates.amount).toFixed(2)) || 0
                    : expense.amount,
              }
            : expense,
        ),
  ...
  const totals: TripBudgetTotals = useMemo(() => {
    const allocatedTotal = DEFAULT_CATEGORIES.reduce(
      (sum, category) =>
        sum +
        (Number.isFinite(allocations[category.value])
  ...
  const categoryTotals: CategoryTotals[] = useMemo(() => {
    return DEFAULT_CATEGORIES.map(({ value, label }) => {
      ...
      const allocation = Number.isFinite(allocations[value])
        ? allocations[value]
        : 0;
