...
export const getTimezoneDayNumber = (
  timestampMs: number,
  offsetMs: number,
): number => Math.floor((timestampMs + offsetMs) / DAY_IN_MS);

export const parseTripDateToDayNumber = (
  dateString: string,
  offsetMs: number,
): number => {
...
export const isFinalTripDayForToday = (
  returnDate?: string,
...
export const groupForecastByDay = (
  ...
  const returnDayNumber = returnDate
    ? parseTripDateToDayNumber(returnDate, offsetMs)
    : null;
  const endDayNumber =
    returnDayNumber !== null
