...
export function WeatherWidget({
  ...
  const emitForecast = React.useCallback(
    (days: ForecastDay[]) => {
      ...
      try {
        ...
        if (
          ...
          if (city) {
            ...
            const queryLocation = locationName
              ? `${city}, ${locationName}`
              : city;
            cacheKey = queryLocation;
            result = await apiService.getCurrentWeather(
              city,
            ...
            result = await apiService.getCurrentWeatherByCoordinates(
              latitude,
              longitude,
            );
          } else {
  ...
  const fetchForecast = React.useCallback(async () => {
    ...
    try {
      ...
      const cached = getCachedWeather(cacheKey, "forecast") as
        | ForecastDay[]
        | ForecastCachePayload
        | null;
      if (cached) {
        const cachedDays = Array.isArray(cached) ? cached : cached?.days;
        const cachedTimezoneOffset = Array.isArray(cached)
          ? undefined
          : cached?.timezoneOffsetSeconds;
      ...
      const departureDayNumber = parseTripDateToDayNumber(
        departureDate,
        offsetMs,
      );
      ...
      if (
        errorMessage.toLowerCase().includes("city not found") ||
        errorMessage.toLowerCase().includes("404") ||
        (errorMessage.toLowerCase().includes("not found") &&
          errorMessage.toLowerCase().includes("weather"))
      ) {
                      ...
                      permissionStatus === "denied" && (
                        <div className="mt-2">
                          <p className="text-xs text-muted-foreground mb-2">
                            Your browser is currently blocking location access.
                            You may need to reset permissions in your browser
                            settings.
                          </p>
                        </div>
                      )}
