...
export function TripPointsOfInterestSection({
  trip,
}: TripPointsOfInterestSectionProps) {
  ...
  const places = allPlaces.filter(
    (poi) => poi.name?.trim() && poi.name.trim() !== "Unknown Place",
  );
        ...
        <Card>
          <CardContent className="py-12 text-center">
            ...
            <p className="text-muted-foreground mb-1">
              We couldn't find any points of interest for {cityName} at the
              moment.
            </p>
            <p className="text-sm text-muted-foreground">
              Don't worry! You can still explore the destination and discover
              hidden gems on your own adventure.
            </p>
          </CardContent>
        </Card>
      </div>
