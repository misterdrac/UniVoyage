...
export function useTripItinerary({
  trip,
  currentStatus,
}: UseTripItineraryArgs): UseTripItineraryResult {
  const { user } = useAuth();
  const [structuredItinerary, setStructuredItinerary] =
    useState<NormalizedItinerary | null>(null);
  ...
  const dateSignature = useMemo(
    () =>
      itineraryDates.length
        ? itineraryDates.map((day) => day.iso).join("|")
        : null,
    [itineraryDates],
  );
    ...
    (snapshot: StoredItineraryPayload | null) => {
      ...
      if (snapshot.structured) {
        setStructuredItinerary(
          normalizeItinerary(snapshot.structured as unknown as GeminiItinerary),
        );
        setRawItinerary(null);
  ...
  const generateItinerary = useCallback(async () => {
    try {
      ...
      const response = await apiService.generateItinerary({
        ...
        itineraryDates: itineraryDates.map((d) => ({
          iso: d.iso,
          label: d.label,
        })),
        userHobbies,
  ...
  const hasStalePlan = Boolean(
    cachedSignature && fullSignature && cachedSignature !== fullSignature,
  );
