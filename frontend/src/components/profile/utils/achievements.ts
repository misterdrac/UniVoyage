...
export const LANGUAGE_TIERS = [
  ...
  { name: "Linguist", threshold: 5, description: "Speak 5+ languages" },
  {
    ...
    threshold: 10,
    description: "Speak 10+ languages",
  },
];
...
export const COUNTRY_TIERS = [
  ...
  },
  {
    name: "UN Ambassador",
    threshold: 193,
    description: "Visit all 193 UN member countries",
  },
];
...
export const MEMBER_TIERS = [
  ...
  { name: "SixSeven", threshold: 24455, description: "Member for 67 years" },
  {
    name: "Immortal",
    threshold: 365000,
    description: "Member for 1000 years (How is this possible?)",
  },
];
...
export const getTierColor = (tier: number): TierColor => {
  return (
    ...
    TIER_COLORS[TIER_COLORS.length - 1]
  );
};
...
export const calculateAchievements = (stats: {
  ...
  const languageTier = getAchievementTier(
    stats.languagesSpoken,
    LANGUAGE_TIERS.map((t) => t.threshold),
  );
  const countryTier = getAchievementTier(
    stats.countriesVisited,
    COUNTRY_TIERS.map((t) => t.threshold),
  );
  const interestTier = getAchievementTier(
    stats.interests,
    INTEREST_TIERS.map((t) => t.threshold),
  );
  const memberTier = getAchievementTier(
    stats.memberFor,
    MEMBER_TIERS.map((t) => t.threshold),
  );
  ...
  return [
    {
      ...
      description:
        languageTier >= 0
          ? LANGUAGE_TIERS[languageTier].description
          : "Speak 2 languages",
      tier: languageTier,
      type: "language",
    },
    {
      ...
      description:
        countryTier >= 0
          ? COUNTRY_TIERS[countryTier].description
          : "Visit 5 countries",
      tier: countryTier,
      type: "country",
    },
    {
      ...
      description:
        interestTier >= 0
          ? INTEREST_TIERS[interestTier].description
          : "Have 2 interests",
      tier: interestTier,
      type: "interest",
    },
    {
      ...
      description:
        memberTier >= 0
          ? MEMBER_TIERS[memberTier].description
          : "Member for 1 month",
      tier: memberTier,
      type: "member",
    },
