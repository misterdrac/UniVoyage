...
export const AchievementBadge = ({ achievement }: AchievementBadgeProps) => {
  const tierColor = achievement.unlocked
  ...
  return (
    ...
    >
      ...
      <div
        className={`relative flex flex-col items-center text-center ${achievement.unlocked ? "" : "grayscale"}`}
      >
        {/* Icon Badge */}
        <div
        ...
        >
          ...
          {achievement.unlocked && tierColor && (
            <div className="absolute -top-1 -right-1">
              <Star
                className={`w-5 h-5 ${tierColor.star} fill-current drop-shadow-lg`}
              />
            </div>
          )}
        </div>
        ...
        {/* Description */}
        <p className="text-sm text-muted-foreground mb-3 min-h-10">
